...
func (r *repository) GetAllPost(ctx context.Context, limit, offset int) (posts.GetAllPostResponse, error) {
	query := `SELECT p.id, p.user_id, u.username, p.post_title, p.post_content, p.post_hashtag FROM posts p JOIN users u ON p.user_id = u.id ORDER BY p.updated_at DESC LIMIT ? OFFSET ? `
	...
	for rows.Next() {
		...
		data = append(data, posts.Post{
			...
			PostHashtag: strings.Split(model.PostHashtags, ","),
		})
	}
	response.Data = data
...
func (r *repository) GetPostByID(ctx context.Context, id int64) (*posts.Post, error) {
	query := `SELECT p.id, p.user_id, u.username, p.post_title, p.post_content, p.post_hashtag, uv.is_liked FROM posts p JOIN users u ON p.user_id = u.id JOIN user_activities uv ON uv.post_id = p.id WHERE p.id = ?`
